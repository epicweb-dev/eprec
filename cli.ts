...
import { normalizeProcessArgs, configureProcessCommand } from "./process-course/cli";
...
import { getDefaultWhisperModelPath, transcribeAudio } from "./whispercpp-transcribe";
    ...
    .command(
      ...
      async (argv) => {
        ...
        const outputBasePath =
          ...
          path.join(path.dirname(inputPath), `${path.parse(inputPath).name}-transcript`);
        const threads =
